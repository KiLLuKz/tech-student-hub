# Science-Math-Technology Program Official Website

> ⚠️ **DEVELOPMENT NOTICE & LEGAL DISCLAIMER**  
> This website is currently **UNDER ACTIVE DEVELOPMENT** for the **Science-Math-Technology Program at Bangpakok Wittayakom School**.  
> 
> **ALL RIGHTS RESERVED.** All contents, source code, school branding, graphic assets, and information within this repository are the exclusive intellectual property of **Bangpakok Wittayakom School** and the development team.  
> 
> **RESTRICTION OF USE:**  
> Unauthorized copying, distribution, modification, extraction, or commercial/non-commercial use of any data, text, media, or code from this repository without prior explicit written permission from Bangpakok Wittayakom School is **STRICTLY PROHIBITED** and may be subject to legal action under applicable copyright laws.

---

## 📌 About the Project
This web application is being built to serve as the official digital showcase for the **Science-Math-Technology Program** at Bangpakok Wittayakom School. It highlights curriculum details, student achievements, academic activities, and program news.

---

## 🛠 Tech Stack

* **Framework:** Next.js
* **Language:** TypeScript / JavaScript
* **Styling:** Tailwind CSS
* **Deployment Platform:** Vercel (Development Pipeline)

---

## 🔒 Intellectual Property & Legal Notice
* **Copyright Notice:** © Bangpakok Wittayakom School. All rights reserved.
* **License:** **No License (All Rights Reserved)**. This project is not open-source. Public visibility on GitHub is intended solely for preview, demonstration, and internal review purposes.
